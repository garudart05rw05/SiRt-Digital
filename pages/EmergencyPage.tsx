...
const EmergencyPage: React.FC<{ role: UserRole }> = ({ role }) => {
  ...
  useEffect(() => {
    const unsubContacts = onSnapshot(doc(db, "app_data", STORAGE_KEYS.EMERGENCY), (docSnap) => {
      if (docSnap.exists()) setContacts(docSnap.data().data);
    });
  ...
  const handleSave = (data: Partial<EmergencyContact>) => {
    ...
    setContacts(updated);
    storage.set(STORAGE_KEYS.EMERGENCY, updated);
    setIsEditing(false);
  ...
  const officials = [
    { name: settings.lurahName || '', title: 'Kepala Kelurahan', phone: settings.lurahPhone || '', icon: '🏛️', color: 'bg-indigo-600', subtitle: 'Pemerintah Kota Kediri' },
    { name: settings.babinsaName || '', title: 'Babinsa', phone: settings.babinsaPhone || '', icon: '🎖️', color: 'bg-emerald-700', subtitle: 'TNI Angkatan Darat' },
    { name: settings.bhabinkamtibmasName || '', title: 'Bhabinkamtibmas', phone: settings.bhabinkamtibmasPhone || '', icon: '👮', color: 'bg-slate-900', subtitle: 'POLRI - Polsek Mojoroto' },
  ].filter(off => off.name !== '' || off.phone !== '');

  if (isEditing) {
    return (
      <div className="px-5 py-6">
        <EmergencyEditor onSave={handleSave} onCancel={() => { setIsEditing(false); setSelectedContact(undefined); }} initialData={selectedContact} />
      </div>
    );
  }
    ...
    <div className="space-y-8 px-5 py-6 pb-32 animate-page-enter">
      ...
      <div className="bg-red-600 rounded-[44px] p-10 text-white shadow-2xl relative overflow-hidden border-4 border-red-500/20">
        <div className="relative z-10 space-y-6 text-center">
          ...
          {role === 'ADMIN' && (
            <button onClick={() => setIsEditing(true)} className="text-white/40 text-[9px] font-black uppercase tracking-[0.3em] hover:text-white transition-colors">Edit Daftar Kontak & URL</button>
          )}
        </div>
        <div className="absolute -right-20 -top-20 w-80 h-80 bg-white/10 rounded-full blur-[100px]"></div>
      </div>

      {/* KARTU UTAMA: KETUA RT - WHATSAPP PRIORITAS */}
      <div className="bg-[#0077b6] p-8 rounded-[44px] shadow-2xl text-white flex flex-col gap-6 border-4 border-blue-500/20 group hover:scale-[1.01] transition-all duration-500">
      ...
      {/* PEJABAT WILAYAH AKTIF */}
      {officials.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between px-2">
             <h3 className="text-[11px] font-black text-slate-400 uppercase tracking-[0.2em] flex items-center gap-2">
               <div className="w-1 h-3 bg-indigo-500 rounded-full"></div>
               Pejabat Wilayah Terdaftar
             </h3>
          </div>
          <div className="grid grid-cols-1 gap-4">
             {officials.map((off, idx) => (
               <div key={idx} className="bg-white border border-slate-100 p-8 rounded-[44px] shadow-sm space-y-6 group hover:border-indigo-100 hover:shadow-xl transition-all duration-500">
                  <div className="flex items-center gap-6">
                     <div className={`${off.color} w-16 h-16 rounded-[24px] flex items-center justify-center text-3xl shadow-lg group-hover:rotate-6 transition-transform text-white`}>
                        {off.icon}
                     </div>
                     <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                           <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{off.title}</p>
                           <div className="bg-emerald-50 px-2 py-0.5 rounded text-[7px] font-black uppercase text-emerald-600 border border-emerald-100 flex items-center gap-1">
                        ...
                        <h4 className="font-black text-slate-800 text-lg leading-tight uppercase tracking-tight truncate">{off.name}</h4>
                        <p className="text-[9px] font-bold text-indigo-400 uppercase tracking-widest mt-1 opacity-70">{off.subtitle}</p>
                     </div>
                  </div>
                  
                  <div className="flex gap-3">
                     <button onClick={() => handleWhatsApp(off.phone)} className="flex-1 bg-emerald-50 text-emerald-600 py-4 rounded-2xl flex items-center justify-center gap-2 font-black text-[10px] uppercase tracking-widest hover:bg-emerald-600 hover:text-white transition-all shadow-sm">
                        <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                        WhatsApp
                     </button>
                     <button onClick={() => handleCall(off.phone)} className="flex-1 bg-slate-900 text-white py-4 rounded-2xl flex items-center justify-center gap-2 font-black text-[10px] uppercase tracking-widest hover:bg-black transition-all shadow-md">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 gap-3">
          {contacts.map(contact => (
            <div key={contact.id} className="bg-slate-50 border border-slate-100 px-8 py-5 rounded-[32px] flex items-center justify-between group hover:bg-white transition-colors shadow-sm">
               <div className="flex items-center gap-5">
                  <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-md text-xl">
                    {contact.provider === 'Keamanan' ? '🚓' : contact.provider === 'Kesehatan' ? '🏥' : contact.provider === 'Kebakaran' ? '🔥' : '⚡'}
                  </div>
                  <div>
                     <h4 className="font-black text-slate-700 text-[10px] uppercase tracking-tight leading-none">{contact.label}</h4>
               ...
               <div className="flex gap-2">
                  ...
                  <button onClick={() => handleCall(contact.phone)} className="p-3 bg-white text-red-600 rounded-2xl shadow-sm active:scale-90 border border-slate-100"><svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg></button>
               </div>
            </div>
          ))}
        </div>
      ...
      <div className="bg-slate-900 p-10 rounded-[56px] text-center space-y-6">
         <div className="flex justify-center -space-x-2">
            <div className="w-12 h-12 rounded-full border-4 border-slate-900 bg-blue-600 flex items-center justify-center text-sm shadow-xl">🏛️</div>
            <div className="w-12 h-12 rounded-full border-4 border-slate-900 bg-emerald-600 flex items-center justify-center text-sm shadow-xl">🎖️</div>
            <div className="w-12 h-12 rounded-full border-4 border-slate-900 bg-slate-700 flex items-center justify-center text-sm shadow-xl">👮</div>
         </div>
         <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.3em] leading-relaxed">
           Seluruh data pejabat di atas diverifikasi berkala oleh Admin {settings.rtRw}.<br/>Gunakan layanan dengan penuh tanggung jawab.
         </p>
      </div>
    </div>
  );
