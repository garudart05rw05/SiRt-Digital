...
interface YouthOfficialsPageProps {
  role: UserRole;
}

const YouthOfficialsPage: React.FC<YouthOfficialsPageProps> = ({ role }) => {
  const [officials, setOfficials] = useState<Official[]>([]);
  const [isEditing, setIsEditing] = useState(false);
  const [selectedOfficial, setSelectedOfficial] = useState<Official | undefined>(undefined);

  useEffect(() => {
    const unsub = onSnapshot(doc(db, "app_data", STORAGE_KEYS.YOUTH_OFFICIALS), (docSnap) => {
  ...
  const handleSave = async (data: Partial<Official>) => {
    ...
    await storage.set(STORAGE_KEYS.YOUTH_OFFICIALS, updated);
    setIsEditing(false);
  ...
  const handleDelete = async (id: string) => {
    if (role !== 'ADMIN') return;
    if (window.confirm('Hapus data pengurus Karang Taruna ini secara permanen?')) {
    ...
    <div className="space-y-8 px-5 py-6 pb-24 animate-page-enter">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-3xl font-black text-slate-800 tracking-tight leading-none uppercase">Pengurus Karang Taruna</h1>
          <p className="text-xs text-violet-500 font-bold uppercase tracking-widest">Generasi Muda Kreatif RT 05 RW 05</p>
        </div>
        {role === 'ADMIN' && (
          <button 
            onClick={() => setIsEditing(true)}
            className="bg-violet-600 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-violet-500/20 active:scale-95 transition-all"
          >
            + Tambah Pengurus
          </button>
        )}
      ...
      {officials.length === 0 ? (
        <div className="py-20 text-center bg-slate-50 rounded-[44px] border border-dashed border-slate-200">
           <p className="text-slate-400 font-black uppercase text-xs tracking-widest">Belum ada data pengurus Karang Taruna.</p>
        </div>
      ) : (
              ...
              <div className="p-8 flex-1 flex flex-col justify-between">
                <div className="space-y-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <span className="bg-violet-50 text-violet-600 text-[10px] font-black uppercase px-3 py-1 rounded-full mb-2 inline-block shadow-sm">
                        {official.position}
                      </span>
                      <h3 className="text-2xl font-black text-slate-800 leading-tight uppercase">{official.name}</h3>
                    </div>
                    {role === 'ADMIN' && (
                      <div className="flex gap-2">
                        <button onClick={() => { setSelectedOfficial(official); setIsEditing(true); }} className="p-2 bg-slate-50 text-slate-400 hover:text-violet-600 rounded-xl transition-all">
                  ...
                  <div className="space-y-2"><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tanggung Jawab</p><p className="text-sm text-slate-500 font-medium">{official.duties || 'Belum ada deskripsi tugas.'}</p></div>
                </div>
                <div className="pt-6 mt-6 border-t border-slate-50 flex items-center justify-between gap-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => window.location.href = `tel:${official.phone}`} className="bg-violet-600 text-white px-4 py-2.5 rounded-2xl text-[9px] font-black uppercase tracking-widest shadow-md">Telepon</button>
...
export default YouthOfficialsPage;
