...
const ResidentList: React.FC<{ role: UserRole }> = ({ role }) => {
  ...
  useEffect(() => {
    const unsub = onSnapshot(doc(db, "app_data", STORAGE_KEYS.RESIDENTS), (docSnap) => {
      if (docSnap.exists()) {
        ...
        storage.updateLocal(STORAGE_KEYS.RESIDENTS, cloudData);
      }
    });
    return () => unsub();
  }, []);
  ...
  const filteredResidents = residents.filter(r => {
    ...
    return name.includes(search) || house.includes(search);
  });
  ...
  const handleSave = async (data: Partial<Resident>) => {
    ...
    const saved = await storage.set(STORAGE_KEYS.RESIDENTS, updated);
    if (saved) {
      setResidents(updated);
      alert(`Database diperbarui! Data warga '${data.name}' berhasil disimpan.`);
      setIsEditing(false);
  ...
  const handleDelete = async (id: string, name: string) => {
    if (role !== 'ADMIN') return;
    if (window.confirm(`Hapus data warga: "${name}"? Tindakan ini permanen dan akan menghapusnya dari server Cloud.`)) {
      const updated = residents.filter(r => r.id !== id);
      const saved = await storage.set(STORAGE_KEYS.RESIDENTS, updated);
      if (saved) {
        setResidents(updated);
        alert(`Data warga "${name}" telah berhasil dihapus dari sistem.`);
      }
    }
  };
  ...
  return (
    <div className="space-y-6 px-5 py-4 pb-24 animate-page-enter">
      <div className="print-header">
        <h1 className="text-2xl font-black uppercase">DATA KEWARGAAN RT 05 RW 05</h1>
        <p className="text-sm font-bold">KELURAHAN GAYAM, KEDIRI</p>
        <p className="text-[10px] mt-1 italic">Laporan Database Terupdate: {new Date().toLocaleDateString('id-ID', { dateStyle: 'full' })}</p>
      </div>
      ...
      <div className="bg-white rounded-[44px] border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-10 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-2 opacity-60">RT Digital Ledger</p>
          </div>
          <div className="flex gap-2 no-print">
            <button 
              onClick={handlePrint}
              className="bg-white text-slate-700 px-6 py-5 rounded-[28px] border border-slate-100 font-black text-xs uppercase tracking-widest shadow-xl active:scale-95 transition-all flex items-center gap-2"
            >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 text-slate-400 text-[10px] font-black uppercase tracking-[0.15em]">
              <tr>
                <th className="px-8 py-6">Nama Warga</th>
                <th className="px-8 py-6">Rumah</th>
                <th className="px-8 py-6 text-center">Usia</th>
                <th className="px-8 py-6">Status</th>
                <th className="px-8 py-6 no-print">Aksi</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {filteredResidents.length === 0 ? (
                <tr><td colSpan={5} className="py-20 text-center text-slate-400 font-black uppercase text-xs tracking-widest">Tidak ada data</td></tr>
              ) : (
                filteredResidents.map((resident) => (
                  <tr key={resident.id} className="hover:bg-slate-50/50 transition-colors group">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-lg font-black shadow-sm no-print ${resident.gender === 'Laki-laki' ? 'bg-blue-100 text-blue-600' : 'bg-pink-100 text-pink-600'}`}>
                    ...
                    <td className="px-8 py-6"><span className="font-bold text-slate-700">{resident.houseNumber}</span></td>
                    <td className="px-8 py-6 text-center"><span className="text-xs font-black text-slate-800">{calculateAge(resident.dateOfBirth)} Thn</span></td>
                    <td className="px-8 py-6">
                      <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider ${resident.status === 'Aktif' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'}`}>
                        {resident.status}
                      </span>
                    </td>
                    <td className="px-8 py-6 text-right no-print">
                      {role === 'ADMIN' && (
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => { setSelectedResident(resident); setIsEditing(true); }} className="p-3 bg-white border border-slate-100 text-[#0077b6] rounded-xl shadow-sm"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" strokeWidth={2.5}/></svg></button>
                          <button onClick={() => handleDelete(resident.id, resident.name)} className="p-3 bg-rose-50 border border-rose-100 text-rose-500 rounded-xl shadow-sm hover:bg-rose-500 hover:text-white transition-all"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" strokeWidth={2.5}/></svg></button>
                        </div>
                      )}
                    </td>
                  </tr>
